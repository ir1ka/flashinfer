...
def _write_scales_decode_strided(cache_tensor, buf, scales, head_dim, layout):
    ...
    scale_stride_f32 = stride // 4
    scale_offset_f32 = head_dim // 4
    batch_size, page_size, num_kv_heads = (
        (cache_tensor.shape[0], cache_tensor.shape[1], cache_tensor.shape[2])
        if layout == "NHD"
        else (cache_tensor.shape[0], cache_tensor.shape[2], cache_tensor.shape[1])
    )
    s = (
        page_size * num_kv_heads * scale_stride_f32,
        num_kv_heads * scale_stride_f32,
...
def test_pth_backend_assertion_prefill_ragged():
    """PTH + non-fa2 backend raises AssertionError in ragged prefill plan."""
    _skip_if_sm_below_75()
...
def test_pth_backend_assertion_prefill_paged():
    """PTH + non-fa2 backend raises AssertionError in paged prefill plan."""
    _skip_if_sm_below_75()
    workspace = torch.empty(128 * 1024 * 1024, dtype=torch.uint8, device="cuda:0")
    ...
    kv_last_page_len = torch.tensor([16, 16], dtype=torch.int32, device="cuda:0")
    with pytest.raises(AssertionError, match="incompatible"):
        wrapper.plan(
            qo_indptr,
            kv_indptr,
...
def test_pth_backend_assertion_decode():
    """PTH + non-fa2 backend raises AssertionError in batch decode."""
    _skip_if_sm_below_75()
    workspace = torch.empty(128 * 1024 * 1024, dtype=torch.uint8, device="cuda:0")
    with pytest.raises(AssertionError, match="incompatible"):
