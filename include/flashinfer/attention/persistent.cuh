...
#pragma unroll 1
    for (IdType work_idx = work_indptr[blockIdx.y]; work_idx < work_indptr[blockIdx.y + 1];
      ...
      [[maybe_unused]] paged_kv_t<DTypeKV, IdType> block_paged_k = params.block_paged_k;
      [[maybe_unused]] paged_kv_t<DTypeKV, IdType> block_paged_v = params.block_paged_v;
...
const void* GetBatchPagedAttentionPersistentKernel(size_t& smem_size, uint32_t& num_threads) {
  return _GetBatchPagedAttentionPersistentKernel<CTA_TILE_Q_1, CTA_TILE_Q_2, HEAD_DIM_QK,
                                                 HEAD_DIM_VO, MASK_MODE, AttentionVariant, Params>(
      smem_size, num_threads);
}
